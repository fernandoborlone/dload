...
describe('Service: Create Users', () => {
  ...
  Cypress._.times(1, () => {
    it('POST - Should create user', () => {
      cy.create_user_api(name, email, password, administrador)
        .then((resp) => {
  ...
  context('Context: Error Messages', () => {
    it('POST - Should display an alert when property name is null', () => {
      cy.create_user_api(name = null, email, password, administrador)
        .then((resp) => {
    ...
    it('POST - Should display an alert when property email is null', () => {
      cy.create_user_api(name, email = null, password, administrador)
        .then((resp) => {
          expect(resp.status).to.eq(400)
    ...
    it('POST - Should display an alert when property password is null', () => {
      cy.create_user_api(name, email, password = null, administrador)
        .then((resp) => {
          expect(resp.status).to.eq(400)
